...
//1.打开题库文件。,返回结果:题库
function tiku() {
    var file = open("./xwf03-main/tk2081.txt", "r", "utf-8"); //读取文件的所有内容
    var text = file.read();
...
function tiku_2() {
    ...
    var path = "./xwf03-main/tk存不会的.txt"
...
window.c.click(function() {
    threads.start(function() {
        engines.execScriptFile("./xwf03-main/0取截图.js"); //导入脚本。
        //  toastLog("题目已保存")
    }); //启动//开一个线程
})
...
window.yijian.click(function() {
    threads.start(function() {
        engines.execScriptFile("./xwf03-main/一键学习.js"); //导入脚本。
        //  toastLog("题目已保存")
    }); //启动//开一个线程
})
