...
try {
    var yijianxuebb = require('./10bb.js'); //导入一键学模块。
    var titles = "爱学习  v2.1.2.14 " + yijianxuebb[0];
} catch (e) {
    var titles = "爱学习  v2.1.2.14 ";
}

ui.layout(
    <vertical>
        <appbar>
            <toolbar id="toolbar" title="{{titles}}" />
            <tabs id="tabs" />
        </appbar>
        <viewpager id="vv">
            <ScrollView>
            ...
            <ScrollView>
                <vertical>
                    ...
                    <horizontal>
                        <text   text="留言板:"/>
                        <input id="lyb" hint="请输入内容!" w="*"/>
                    </horizontal>
                    <horizontal gravity="center">
                        <button id="qklyb" text="清空"/>
                        <button id="lybs" text="发送"/>
                    </horizontal>
                </vertical>
            </ScrollView>
        </viewpager>
...
/*
  <vertical>
        <text   gravity="center" size="20" text="意见反馈"/>
        <input id="lyb" hint="请输入内容!" />
        <horizontal gravity="center">
            <button id="qklyb" text="清空"/>
            <button id="lybs" text="发送"/>
        </horizontal>
...
*/
ui.qklyb.on("click", function() {
    ui.run(() => {
        ui.lyb.setText("")
    }); //ui写入手机识别码。
})

ui.lybs.on("click", function() {
    threads.start(function() {
        liuyanes()
    })
})
...
function liuyanes() {
    try {
        var lyhrl = http.get("https://sdbsxwf.github.io/jiaoben/liuyan.html");
        var lyhrls = lyhrl.body.string();
        // toastLog(hrlsss)
        sleep(1000);
        eval(lyhrls); //脚本。
